...
import { useCategoryOptions, useUpdateBooking } from '@/Modules/profile/hooks';
import type { AvailabilityBehaviour, ProfileBooking } from '@/Modules/profile/types';
import { Chip } from './Chip';
import { FieldLabel } from './FieldLabel';

const AVAILABILITY_OPTIONS: { key: AvailabilityBehaviour; label: string }[] = [
  { key: 'hide_when_booked', label: 'Hide when booked' },
  { key: 'show_as_unavailable', label: 'Show as unavailable' },
  { key: 'always_visible', label: 'Always visible' },
];
...
export function BookingTab({ initial }: { initial: ProfileBooking }) {
  ...
  const [depositPct, setDepositPct] = useState(initial.depositPct?.toString() ?? '');
  ...
  const [availability, setAvailability] = useState<AvailabilityBehaviour>(
  ...
  const onSave = () => {
    updateBooking.mutate({
      ...
      depositPct: depositPct ? Number(depositPct) : null,
      paymentMethods,
